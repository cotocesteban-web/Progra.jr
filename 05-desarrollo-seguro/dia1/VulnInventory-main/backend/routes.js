...
router.post('/auth/login', async (req, res) => {
 ...
 // 1. Cambiamos las variables directas por los signos de interrogación (?)
const query = 'SELECT id, username, role FROM users WHERE username = ? AND password = ?';

try {
  // 2. Pasamos las variables limpias dentro de un arreglo [] usando querySafe
  const users = await db.querySafe(query, [username, password]);
